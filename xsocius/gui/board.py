...
class PresentationBoardMixin:
    ...
    # Override this for "demo" behavior (used in prefs panel to show
    # appearance of a mini-board
    demo = False

    # In this mixin, we can't assume that self=puzzle window, so pass
    # puzzle to our methods
    ...
    def PrepareDrawSizing(self, bufwidth, bufheight, startx=0, starty=0):
        ...
        # The size of the graphic board is almost certainly smaller than the
        # allocated space (since every rect has to be the same size). Calculate
        # Leftover space and use this to center game board in space.
