...
# Process of sharing:
#
# One computer ("sharer") opens a puzzle normally then chooses to share it. They are prompted for
#  username/password for XMPP server. An XMPP connection is made and stored as .xmpp.
# Authentication proceeeds. Once authenticated, they can send an invite to a friend; this is
# essentially just a text message that gives the JID of the sharer. Sending the invite is
# optional and whether it is sent or not does not affect the process.
#
# The other computer ("joiner") may or may not have a puzzle open, but, if so, it won't be the
# shared puzzle, anyway. When they know they want to join a puzzle (perhaps they received an
# invite IM in their normal IM software), they choose to join from the menu. This promps for XMPP
#  connection info (same dialog as sharer). They store the .xmpp connection and start
# authentication. Once authenticated, they are prompted for the JID of the sharer. This JID is
# stored as .friend and .friendnick.  (If they already have the request-JID dialog open when the
# specially formatted invite text message is sent, this is noticed and things proceed as if they
# manually entered the JID). Either case, they then send a "HELLO" command to the sharer to let
# them know they're ready.
#
# When the sharer receives the HELLO, they now know the ID of their "friend"; this is stored as
# .friend and .friendnick. They then send a FILE command, letting the joiner know to prepare to
# receive the initial puzzle file. This is followed by sending the file in a data packet to the
# joiner.
#
# When the joiner receives the FILE command, they know that the connection attempt was successful
#  (and hide the "waiting connection" dialog). They get the file and open it in a new window.
# Since the window they had before isn't really related to sharing, .xmpp, .friend,
# and .friendnick are removed from it and set on the new window.
#
# At this point, the two machines are now in the same state and act the same-- the can send
# messsages, SET moves, CLEAR, CHECK, REVEAL, and HIGHLIGHT to each other. A machine can send
# DISCONNECT to the other machine to inform it of its intent to disconnect.

# 2.0 future goal: the sharer should now be able to share with more than one person; friend -> [
# friends], friendnick -> [friendnick], and we should iterate over the list of people to send
# things like messages to. In addition, when we are the sharer and receive a message, we should
# iterate over people that aren't the sender to rebroadcast it.
...
def addLogOnDialogOptions(self, config):
    ...
    # Since we ask the same server/user/password questions both during the sharing process and in
    #  the preferences panel for sharing, this is factored out for use in both places.
...
class XMPPWaitDialog(wx.Dialog):
    """Dialog box that shows we're waiting to get puzzle from sharer.
       ...
       - manually canceled (user got bored waiting for reply); ultimately fires off disconnection

       - system calls EndModal passing ID_CANCEL (got an error while trying to join a puzzle);
       ultimately fires off disconnection

       - system calls EndModal passing ID_OK (got notice it would get the puzzle file).
    """
...
class JoinFriendDialog(wx.Dialog):
    """Dialog box that prompts to join a user.
       ...
       - system calls EndModal with JID of friend; this is done if we actually get the invite
    """
...
class MsgEntry(wx.TextCtrl):
    ...
    def OnEnter(self, event):
        ...
        # If user has preference set, focus returns to the board after every message -- but in
        # any case, if they press return on a blank message, focus returns to the board.
...
class ShareWindowMixinBase:
    ...
    In order to provide the functionality needed for sharing, the GUI Puzzle object subclasses
    sharing-specific mixins. This mixin is a base of those behaviors that are same whether this
    is the sharer or joiner.
    """

    # Underlying XMPP connection object. This is used both as the interface to the XMPP system (
    # ie, things are called on the object), but also as a marker for us to know that there is a
    # connection -- if this is None, there's no connection.
    xmpp = None

    # Friends we're connected to will get connected to this as a dict. For joiners, this will
    # only ever be one person; for sharers, it can be more than one.
    #
    #   { 'joel@server.com/resource': Friend(..), ... }
    # 

    # Pointer to cancelable dialog showing joining is awaiting file send from sharer.
    ...
    def XMPPConnect(self, server, username, password, is_sharer):
        ...
        if self.xmpp is not None:
            # Must be re-joining with /rejoin command -- so let's keep the existing friend info.
        ...
        if is_sharer:
            # Attach connection to logical puzzle object (if we're not a sharer, there may not be
            #  a puzzle yet--this could be the dummy window)
            self.puzzle.xmpp = self.xmpp

        # All things being equal, we'd rather not reattempt connections, but it appears sleekxmpp
        #  first tries an IPv6 lookup for google talk and, if we don't allow retry, we'll never
        # get the IPv4 connection.
    ...
    def XMPPDisconnect(self, msg="Goodbye", noecho=False,
        ...
        # We get an exception if we later try to actually do an xmpp disconnect while it's
        # already happening, so let's use a flag to prevent calling this while we're already
        # disconnecting.
        ...
        if self.xmpp:
            # XXX under what circumstances could this be called where there is no self.xmpp?
            try:
                # It's tempting to take the wait=True out of the next line to make disconnections
                #  faster--but this isn't waiting for the disconnection to finish, it's waiting
                # to send queued messages before the disconnection happens. There doesn't seem to
                #  be way to do async disconnects.
    ...
    def XMPPReceiveMsg(self, msg, mfrom=None):
        ...
        if JOIN_MSG in msg and self.joining_dialog_open:
            # We're the joiner and we currently are awaiting the invite and this is it. Fill the
            # information out in the dialog and press OK -- just like the human would if this
            # were manually done.
    ...
    def XMPPSetCell(self, x, y, val, rebus):
        ...
        # This might have just completed the puzzle or finished a clue, so let's make sure those
        # kinds of things are checked for.
        #
        # XXX except that setResponse already calls add_undo which calls this.
        #
    ...
    def XMPPCheckCells(self, cells):
        ...
        if cells == ["*", "*"]:
            ...
            if self.puzzle.check_puzzle(noecho=True):
                # Something was found as an error, so highlight entire board (technically,
                # given that we're called, something must have been found, since this request
                # isn't sent out unless an error was found. But being cautious.)
                flag = True
    ...
    def XMPPRevealCells(self, cells):
        ...
        if any_changed:
            ...
            # This might have solved the puzzle or changed which clues are completed; check for
            # this kind of stuff.
            self.puzzle.on_any_change()
    ...
    def XMPPHighlight(self, cells):
        """Highlight the cells.
           ...
           This is called in response to a %HIGHLIGHT command--it means to highlight the cells
           and leave them lit up (no de-highlighting is scheduled).
        """
...
class ShareWindowMixin(ShareWindowMixinBase):
    ...
    def XMPPFriendInvite(self, buddies):
        """Offer to send invitation out.
           ...
           Performed only by the sharer. This will send a single textual IM to the potential
           friend. This is not the full JID, so it will appear in clients like GTalk.

           It is not required that this be sent out or received--the sharer could communicate the
           JID via offline means. This is just a convenience.
        """
        ...
        if SKIP_UI:
            # For developer testing purposes, we just send an invite to ourself
            config = wx.GetApp().config
            self.xmpp.send_invite(config.username)
            self.XMPPShowSharePanel()
            return

        # We may have IDs & names for buddies, or just IDs. Sort the list by name-or-ID.
        ...
        # Whether or not they send an invite, they're still going into sharing mode (they might
        # want to send an invite manually) so show the sharing panel.
...
class JoinWindowMixin(ShareWindowMixinBase):
    """Mixin for puzzles for joining functionality.
       ...
       Join can be used even when a puzzle is not open; if used when a puzzle is opened,
       another window would be opened for joined puzzle, anyway (just like File -> Open)
    """
    ...
    def XMPPJoinFriend(self):
        """Request friend connection ID.
           ...
           Performed only by joining computer, the user must enter the JID for the sharer. This
           is stored as the friend and a %HELLO greeting is sent, which will initiate the puzzle
           transfer and play.
        """
        ...
        while True:
            ...
            # Show the join-your-friend dialog; we will be passing in an error message and the
            # JID-entered before if this loops a second or more time.

            # This dialog can be closed by the user clicking OK or cancel but it can also be
            # closed programmatically, if we hear an invite code while it's open. Therefore,
            # we attached the dialog to ourself as joining_dialog_open so that if that invite
            # comes across, we can fill it out and press OK in code.
            ...
            if result == wx.ID_OK:
                # If they did a bad copy-paste from the invite sentence, they may have left a
                # trailing person at end.
                ...
                if not friend.nick:
                    ...
                    # Actually try to connect with them; this is asyncronous, so it returns
                    ...
                    # Show a dialog saying that we're joining and set a flag during the
                    # waiting-for-join process. When the join has completed, code elsewhere will
                    # close the modal dialog and this will continue.

                    self.xmpp.tentative = True
                    ret = self.joining_wait_dialog.ShowModal()
                    logging.info("post joining_wait_dialog")

                    # The user might cancel the dialog directly, but typically, we wait here
                    # until the system uses EndModal (ID_OK) to show that the file is being sent
    ...
    def XMPPJoined(self, orig_filename, puzzle_data):
        ...
        We've joined a puzzle and via xmpp have received the data for the puzzle. Save it to disk
        in our crosswords directory and then open it up.

        Whatever window is already open (dummy or a real puzzle), it's not the puzzle we just
        received over the wire, so let's move the .xmpp connection from the current window and
        attach it to the new window with the puzzle we're now sharing.
        """

        # If this is a rejoin, we didn't ask for friend ID and so we might still have the
        # connecting-busy-box up.
        self.close_busy_box()
        ...
        if not self.dummy and self.share_panel.IsShown():
            # We're currently on a shared crossword puzzle--so we must be rejoining. Let's close
            # the sharing panel on this puzzle
...
class Connection(sleekxmpp.ClientXMPP):
    """XMPP connection object.
       ...
       This handles all of the actual wire communication. The wx app can call methods directly on
       this. When this wants to notify the wx app of things, it will use wx.CallAfter to request
       a wx call.
    """
    ...
    def __init__(self, jid, password, wxc, is_sharer, invisible):
        ...
        This is the logical connection the xmpp process. Just because we're creating this doesn't
        mean we're actually logged onto a server--that's the connect() call on this object.
        ...
        # Add pointer to the WX object. We can use this to refer to the GUI from within the XMPP
        # threads. This object is the puzzle window.
        #
        # We SHOULD NOT use this for anything except binding wx.CallAfter() to callable and for
    ...
    def ssl_invalid_cert(self, event):
        ...
        # Lots of people have a Jabber JID that isn't related to the hostname of the jabber
        # server (ie, my JID is "joel@joelburton.com" and the hostname is "talk.google.com" (
        # Google Apps account).
    ...
    def session_start(self, event):
        """Server connection has been successfully made.
           ...
           The sharer will request a roster (which, when returned, will ultimately be shown to
           use for an invite).
    ...
    def roster_update(self, roster):
        ...
        The sharer get this when xmpp sends the list of buddies; fix it up and call wx to put up
        the send-invite-to-friend dialog.
        """
    ...
    def message(self, message):
        """XMPP message received handler.
           ...
           Messages are used for almost all gameplay commands as well as IMs between players.

           Commands all start with %COMMANDNAME. IMs cannot start with a %.
        """
        ...
        if message['type'] == "error":
            ...
            if self.tentative:
                ...
                if not self.wxc.friends:
                    ...
                    if not SKIP_UI:
                        # If this is during testing, though, just skip -- since we can't actually
                        #  send an invite to ourselves and that's what developer testing mode
                        # tries to do
                        wx.CallAfter(self.wxc.XMPPNotifyBadInvite)

                elif self.wxc.joining_dialog_open is not None:
                    # We're the joiner, and we entered a bad invite code. Nothing required to do
                    # here, it already knows the code didn't work
                    pass
        ...
        # If we're the sharer, it's our responsibility to rebroadcast messages to friends who
        # didn't send us this. We don't rebroadcast connection stuff, like %HELLO or %DISCONNECT,
        #  but do want to send along normal instant messages and other commands
        rebroadcast = True
        ...
        if body.startswith('%'):
            ...
            if cmd == "%HELLO":
                ...
                self.recv_clear(data)
            elif cmd == "%CHECK":
                self.recv_check(data)
            elif cmd == "%REVEAL":
                self.recv_reveal(data)
            elif cmd == "%HIGHLIGHT":
                self.recv_highlight(data)
    ...
    def send_message(self, message, only_to=None):
        ...
        Normally, we send everything to everyone. However, in some cases, we want to only send a
        ...
        if only_to:
            ...
            for friend in self.wxc.friends:
                ...
                # It seems as if google throttles us if we go too fast and messages are sent back
                # to us (erk)
                #
                # perhaps this won't be a problem with normal people or maybe it will -- and
                # we'll need to have a pipeline in the object that wx writes to and then
                # self-throttles sending it out for now, just slow down a bit.
                #
    ...
    def handle_data(self, event):
        """Handle receipt of puzzle by joiner.
           ...
           Normal commands and IMS are sent as messages; this is used just for sending the
           initial puzzle.

           Puzzles are sent as base64'd bzipped .puz files; because there is a size limit to data
           packets, it may be split into multiple messages--the last message has \n\n%END at the
           end.
        """
        ...
        if self.filename:
            if data_chunk.endswith(b"\n\n%END"):
                ...
                # Join all chunks together and remove end marker.
    ...
    def send_hello(self):
        ...
        if not self.invisible:
            # Now that we're greeting our sharer, we no longer need to keep a presence on xmpp --
            #  so we can switch back to being unavailable. Note that if we were also logged into
            # xmpp via another client [like gchat], we'll still be available, as that's a
            # different jabber resource.
            #
            # Don't do this if we are in invisible mode, since we never changed our presence to
            # available anyway.
    ...
    def recv_hello(self, data):
        ...
        # Note that we've started sending the file but haven't yet been fully joined by the
        # joiner. XXX doesn't seem to be used self.invited_waiting = False

        if not self.invisible:
            # Now that we're greeting our joiner, we no longer need to keep a presence on xmpp --
            #  so we can switch back to being unavailable. Note that if we were also logged into
            # xmpp via another client [like gchat], we'll still be available, as that's a
            # different jabber resource.
            #
        ...
        # Push changes we've made to puzzle down the file level so we're sending the up-to-date
        # puzzle
    ...
    def send_file(self, friend, filename, data):
        """Send file.
           ...
           Initially, %FILE <puzzlepath>. This is followed by a data send of the puzzle,
    ...
    def recv_file(self, data):
        """Receive %FILE and prepare for file reception.
           ...
           Performed only by slave for receiving puzzle. Command is %FILE <filename>.

           Note that this isn't the receipt of the actual file-- the file command just tells us
           the puzzle name and to prepare to receive the file as a separate xep_0047 data transfer.
        """

        # Note what the filename is and prepare to receive it; the actual file data is handled in
        #  handle_data, above.

        logging.info("%FILE recv")

        # We now know that the joiner recognizes us, so we can get rid of the waiting-to-join
        # dialog box.
