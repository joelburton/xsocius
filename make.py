...
# 
# Once that is done, then you run the FTP script, normally as:
#
...
#
# You can FTP right from this script by passing in the -m option for the upgrade message. This
# builds then FTPs.
#
...
# building the Windows build if you've already run the Mac build--since the docs have already
# been made.
#
...
def build(name, fast=False, make_docs=True):
    ...
    if fast:
        ...
        # We want a zip file of optimized .pyo files that can be used without the corresponding
        # .py files -- since this is how py2app and py2exe will make things.
        # 
        # In py2.7, this could be done with
        #   python setup.py build_py -O2
        #
        # However, in py3.3, this puts things in __pycache__ -- and that requires that we have
        # the original .py files (as well as not matching how py2app/py2exe do things.
        #
        # So instead we now use compileall with the -b (legacy pyo locations) option. This is
        # called on a copy of the xsocius lib put into build/lib.
        ...
        # Copy the app to the upload directory (use ditto for copying because Python's copyfile
        # will lose the resource fork.
