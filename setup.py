...
#
# This is called by make.py to make the apps/eggs; that also does other
# things, like build the help system and put the final files into the
# right upload/ directory for uploading to an FTP server.
#
# This bypasses that if used directly.
#
# Note: this is also used as "setup build_ext" to build cython-based
# unlocker.
...
# SKIP_UI is only for developer testing of the sharing feature; we
# don't want to accidentally release a version with it turned on.
...
def BuildOSXApp():
    ...
    # py2app packages up all of the libraries for Python into a zip file.
    # We need to fix it up.
    #
    # There are two problems with it:
    #
    # - It contains junk we don't need in it (Vim swap files, and non-py
    #   resources for Xsocius
    #
    # - It contains the dylib for wxPython but in the wrong place
    #
    # Unzip this file to a temp directory, fix up the files, re-zip it up,
    # and replace the original zip with the improved one.
    ...
    # Now, re-zip up, exlcluding resources & dylibs that shouldn't be
    # the lib zip
    ...
    # At this point, we have a functional OSX app -- however, it's rather
    # bloated, as it contains code for alternate architectures (like i386)
    # We'll use "ditto" to strip out the other architectures, making the
    # app about half the size.
    #
    # Then, we use hdiutil to make a .dmg of it, which is used by users
    # to install the app.
    #
...
def BuildWindows():
    ...
    # cx_Freeze can make installers with "bdist_msi", but they are
    # too simple--they don't offer uninstallers, and don't put the
    # program in the Start Menu. So we make it with InnoSetup.
...
if __name__ == "__main__":
    ...
    if cmd == "py2app":
        ...
        # Keep this here so we can get help on setup.py and perhaps
        # use any other commands (which aren't needed for the standard
        # build process
        setup()
